...
3. On success you get a count of features/test cases/steps generated and the output path. Files that aren't per-feature reports (e.g. an overview/summary page) are silently skipped; files that fail to parse are listed as warnings rather than failing the whole run. A `method` step that never actually fired a request (e.g. it was skipped because an earlier step in the scenario failed first, so Karate recorded no request/response logs for it at all) is likewise omitted rather than turned into a step with nothing to execute — noted as a warning, not a failure. If a step's logs are present but don't match the expected format closely enough to rebuild a curl command, that step is still included with a `# Could not reconstruct the request for step: ...` placeholder in Test Data, also flagged as a warning so it's easy to spot which step needs a manual look.
