...
4. The test flips to `recorded` and shows its dashboard: sample count, error rate, actual vs. planned TPS/duration, and average/p95/p99 response time as stat tiles, plus three charts — response time (avg + p95), throughput (TPS), and server CPU/RAM — all on a shared "minutes elapsed since the test started" x-axis. Hover any chart for exact values at a point; each also has a "Table view" with the same data. A result can't be re-recorded once uploaded — add a new planned test if you need to run it again.
